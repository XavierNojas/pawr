...
class PawRAppSupabaseUser extends BaseAuthUser {
  PawRAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pawRAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PawRAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
