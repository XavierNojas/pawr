...
import 'log_mood_list_model.dart';
export 'log_mood_list_model.dart';
...
class LogMoodListWidget extends StatefulWidget {
  final Pet pet;
  const LogMoodListWidget({Key? key, required this.pet}) : super(key: key);

  static String routeName = 'LogFood';
  static String routePath = '/logFood';

  @override
  State<LogMoodListWidget> createState() => _LogMoodListWidgetState();
}

class _LogMoodListWidgetState extends State<LogMoodListWidget>
    with TickerProviderStateMixin {
  late LogMoodListModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => LogMoodListModel());

    _model.searchFieldTextController ??= TextEditingController();
    _model.searchFieldFocusNode ??= FocusNode();

    final moodLogVM = Provider.of<PetViewModel>(context, listen: false);
    moodLogVM.fetchMoodLogs(widget.pet.id);
  ...
  Widget cardCreator(BuildContext context) {
    return Consumer<PetViewModel>(builder: (context, moodLogVM, child) {
      if (moodLogVM.isLoading) {
        return const Center(
          heightFactor: 10.0,
      ...
      /// Dynamically create models only when moodLogs load or change
      if (_logMoodCardModels.length != moodLogVM.moodLogs.length) {
        // Dispose old models
        for (var model in _logMoodCardModels) {
          model.dispose();
        }
        // Create new models
        _logMoodCardModels = moodLogVM.moodLogs
            .map((_) => createModel<MoodLogCardModel>(
                context, () => MoodLogCardModel()))
            .toList();
      }

      return moodLogVM.moodLogs.isEmpty
          ? Center(
              child: Text(
                'No moodLogs for ${widget.pet.name}',
                style: FlutterFlowTheme.of(context).titleSmall.override(
                      fontFamily: 'Manrope',
                      color: FlutterFlowTheme.of(context).secondary,
                      fontSize: 16.0,
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                return cardTemplate(context, moodLogVM.moodLogs[index],
                    _logMoodCardModels[index], index, moodLogVM);
              },
            );
    });
  }
  ...
  Widget cardTemplate(
      BuildContext context, MoodLog mood, MoodLogCardModel model, int index, PetViewModel moodLogVM) {
    return Padding(
        ...
        children: [
          wrapWithModel(
            ...
            child: MoodLogCardWidget(
              moods: mood.mood,
              description: mood.observation,
              cardId: mood.petId,
              createdDate: _model.formatDateTime(mood.created_at ?? '2015-01-01 03:33:33.405336+00'),
            ),
