...
import 'transactions_log_other_accepted_model.dart';
export 'transactions_log_other_accepted_model.dart';
...
class TransactionsLogOtherAcceptedWidget extends StatefulWidget {
  final bool? isFromProfile;
  const TransactionsLogOtherAcceptedWidget({Key? key, required this.isFromProfile}) : super(key: key);

  static String routeName = 'LogFood';
  static String routePath = '/logFood';

  @override
  State<TransactionsLogOtherAcceptedWidget> createState() => _TransactionsLogOtherAcceptedWidgetState();
}

class _TransactionsLogOtherAcceptedWidgetState extends State<TransactionsLogOtherAcceptedWidget>
    with TickerProviderStateMixin {
  late TransactionsLogOtherAcceptedModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => TransactionsLogOtherAcceptedModel());
    ...
    requestVM.fetchOtherAcceptedRequests();
  ...
  Widget cardCreator(BuildContext context) {
    return Consumer<RequestViewModel>(builder: (context, requestVM, child) {
      ...
      if (_transactionCardModels.length != requestVM.otherAcceptedRequests.length) {
        // Dispose old models
        for (var model in _transactionCardModels) {
          model.dispose();
        }
        // Create new models
        _transactionCardModels = requestVM.otherAcceptedRequests
            .map((_) => createModel<TransactionCardModel>(
                context, () => TransactionCardModel()))
            .toList();
      }

      return requestVM.otherAcceptedRequests.isEmpty
          ? Center(
              child: Text(
