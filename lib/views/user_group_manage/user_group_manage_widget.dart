...
import 'user_group_manage_model.dart';
export 'user_group_manage_model.dart';
...
class UserGroupManageWidget extends StatefulWidget {
  const UserGroupManageWidget({super.key});

  static String routeName = 'PetManage';
  static String routePath = '/petManage';

  @override
  State<UserGroupManageWidget> createState() => _UserGroupManageWidgetState();
}

class _UserGroupManageWidgetState extends State<UserGroupManageWidget>
    with TickerProviderStateMixin {
  late UserGroupManageModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => UserGroupManageModel());
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Text(
                        'Referenced users will appear here',
  ...
  Widget cardCreator(BuildContext context) {
    return Consumer<FriendsViewModel>(builder: (context, friendsVM, child) {
      ...
      if (_FriendCardModels.length != friendsVM.myFriendsDetails.length) {
        // Dispose old models
        for (var model in _FriendCardModels) {
          model.dispose();
        }
        // Create new models
        _FriendCardModels = friendsVM.myFriendsDetails
            .map((_) =>
                createModel<FriendCardModel>(context, () => FriendCardModel()))
            .toList();
      }

      return friendsVM.myFriendsDetails.isEmpty
          ? Center(
              child: Text(
                'No referrenced users',
                style: FlutterFlowTheme.of(context).titleSmall.override(
                      fontFamily: 'Manrope',
                      color: FlutterFlowTheme.of(context).secondary,
                      fontSize: 16.0,
          ...
          : ListView.builder(
              ...
              itemCount: friendsVM.myFriendsDetails.length,
              itemBuilder: (context, index) {
                return cardTemplate(context, friendsVM.myFriendsDetails[index],
  ...
  Widget cardTemplate(BuildContext context, UserDetails friend, FriendCardModel model,
      int index) {
    return Padding(
  ...
  Future<void> fetchMultipleFriends() async {
    ...
    await friendsVM.fetchMultipleFriends();
